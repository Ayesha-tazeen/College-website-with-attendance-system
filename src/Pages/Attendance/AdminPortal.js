        ...
        <>
            <div className="container w-full mt-5 rounded-lg px-3 py-9 bg-gray-100  md:w-[75%] mx-auto">
                ...
                <div className="mb-4  flex justify-between items-center">
                    ...
                    <div>
                        <button onClick={toggleDeleteButtons} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded">
                            {showDeleteButtons ? 'Close editing list' : 'Edit students list'}
                        </button>
                    </div>
                </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr>
                            ...
                            {showDeleteButtons && <th className=" border border-gray-400 bg-yellow-200 text-blue-950 py-2"></th>}
                        </tr>
                    </thead>
                    <tbody>
                        {students.map((student, index) => (
                            <tr key={index}>
                                <td className="border border-gray-700 px-2 py-2">  {student.rollNumber}  </td>
                                ...
                                <td className="border border-gray-700 px-4 py-2">
                                    <select
                                        ...
                                        className={`border focus:outline-none font-bold text-white focus:none focus:border-none rounded px-3 py-2 w-full ${attendance[index] === 'present' ? 'bg-green-400' : attendance[index] === 'absent' ? 'bg-red-400' : 'bg-blue-400'}`}
                                    >
                                ...
                                {showDeleteButtons && (
                                    <td className="border border-gray-700 py-2">
                                        <button onClick={() => handleDeleteStudent(index)} className="bg-white  text-white px-1 py-2 rounded">
                                        ❌
                                        </button>
                                    </td>
                                )}
